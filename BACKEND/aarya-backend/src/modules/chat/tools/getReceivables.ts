...
export const getReceivables = tool(
  async (input, config) => {
    ...
    try {
      ...
      return JSON.stringify({
        success: true,
        data: {
          totalReceivablesAmount: Number(totalAmount.toFixed(2)),
          count: list.length,
          dueAfter: dateFilter,
          receivables: list,
        },
      ...
      return JSON.stringify({
        success: false,
        error: `Unexpected error retrieving receivables: ${err.message || String(err)}`,
      });
    }
  },
  {
